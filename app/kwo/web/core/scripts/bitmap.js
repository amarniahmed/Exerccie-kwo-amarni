...
Kwo.BitmapEditor = Class.create({
  ...
  initialize: function(elt) { 
    ...
    this.elt.addEventListener("click", function(evt){ 
      ...
      var pointer = Event.pointer(evt);      
      var ratio = $("bitmap-zoom").readAttribute("data-zoom");
      $("mouse-x").update(Math.round((pointer.x - offset.left) / ratio));
      $("mouse-y").update(Math.round((pointer.y - offset.top) / ratio));
    }, true);
  ...
  onCrop: function(x, y, width, height) {
    var args = {"operation": "crop",
                ...
                "ratio": $("bitmap-zoom").getAttribute("data-zoom") };
    this.onExec(args);
